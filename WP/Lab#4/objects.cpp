...
bool Circle::Move(const HDC &hdc,const RECT& rect,HBRUSH &hBrush) {
    ...
    //The drawing
    Ellipse(hdc,center.x-10,center.y-10,center.x+10,center.y+10);
    //Reseting the brush
...
bool Objects::Collision(const RECT &rect) {
    ...
    if(center.x+10>rect.right-1)  {xSpeed=-abs(xSpeed);}
    if(center.x-10<rect.left+1)   {xSpeed=abs(xSpeed);}
    if(center.y+10>rect.bottom-1) {ySpeed=-abs(ySpeed);}
    if(center.y-10<rect.top+1)    {ySpeed=abs(ySpeed);}
